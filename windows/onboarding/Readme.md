...
- Required operator inputs:
  ...
  - Temporary password meeting complexity rules
...
- Validation signals: Success messages indicate created account and mode used; warnings indicate duplicate names/usernames, missing manager/source user, or invalid OU.

## NewUserOrCopyFromCsv.ps1

- Purpose: Create AD users in bulk from a CSV using either new-user (`N`) or copy-user (`C`) action per row.
- Required operator inputs:
  - CSV file path prompt
...
- File path behavior: Creates `C:\Temp` if missing; reads update CSV from `C:\Temp`; writes run log, transcript, and backup CSV to `C:\Temp`.
- Key commands/functions used: `Import-Module ActiveDirectory`, `Import-Csv`, `Get-ADUser`, `Set-ADUser`, `Export-Csv`, `Start-Transcript`, `Write-Progress`.
